          ...
          {({ zoomIn, zoomOut }) => (
            <>
              <div
                ref={canvasRootRef}
                className={cn(
                  `absolute inset-0 w-full h-full bg-[#f5f3f0]
                  dark:bg-[#111018] p-3
              `,
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle, oklch(0.55 0.02 260 / 0.15) 0.6px, transparent 0.6px)",
...
function CanvasLoader({
  ...
  return (
    <div
      ...
      max-w-full px-5 py-2.5
      rounded-full shadow-lg backdrop-blur-md
      flex items-center space-x-2.5 z-20
      border
    `,
        status === "fetching" && "bg-muted/80 text-foreground border-border/60",
        status === "running" && "bg-amber-500/90 text-white border-amber-400/50",
        status === "analyzing" && "bg-blue-500/90 text-white border-blue-400/50",
        status === "generating" && "bg-primary/90 text-primary-foreground border-primary/50",
        status === "finalizing" && "bg-primary/90 text-primary-foreground border-primary/50"
      )}
    >
